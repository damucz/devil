...
ILboolean iLoadRotInternal(void)
{
	...
	FormLen = GetBigUInt();
	iread(FormName, 1, 4);
    if (strncmp((const char*)Form, "FORM", 4) || FormLen != 0x14 || strncmp((const char*)FormName, "HEAD", 4)) {
    ...
    if (strncmp((const char*)Form, "FORM", 4) || strncmp((const char*)FormName, "MIPS", 4)) {
		ilSetError(IL_INVALID_FILE_HEADER);
		return IL_FALSE;
	}
	...
	do {
		...
		MipSize = GetBigUInt();
		iread(FormName, 1, 4);
        if (strncmp((const char*)Form, "FORM", 4)) {
        ...
        if (strncmp((const char*)FormName, "MLVL", 4)) {
			ilSetError(IL_INVALID_FILE_HEADER);
			return IL_FALSE;
		}
